#python model.py car_racing --filename ./controller/carender_moder_racing.cma.4.32.best.json --render_mode --record_video
...
from vae.arch_custommaze import VAE
from rnn.arch_custommaze import RNN
from controller.arch_custommaze import Controller
...
def make_model():
  ...
  vae = VAE()
  vae.set_weights('./vae/vae_weights/weights_custommaze_roll35000.5z32.ckpt')

  rnn = RNN()
  rnn.set_weights('./rnn/rnn_weights/weights_custom2dmaze_roll4.50.100.ckpt')
...
class Model:
  def __init__(self, controller, vae, rnn):
    ...
    self.output_size = controller.output_size#3
  ...
  # def make_env(self, env_name, seedmake_env=30, render_mode=False, model = None):
  def make_env(self, env_name, seedmake_env=-1, render_mode=False, model = None):
    self.render_mode = render_mode
    self.env_name = env_name
    # print("seedmake_env",seedmake_env)
    self.env = make_env(env_name, seed=seedmake_env, render_mode=render_mode, model = model)
  ...
  def update(self, obs, t):
    if obs.shape == self.vae.input_dim:#(64,64,3)
      return self.vae.encoder.predict(np.array([obs]))[2][0]#[obs](1,64,64,3)
...
def simulate(model, num_episode=5, seedsim=-1, max_len=-1, generate_data_mode = False, render_mode = False):
# def simulate(model, num_episode=5, seedsim=20, max_len=-1, generate_data_mode = False, render_mode = False):
  reward_list = []
  t_list = []
  ...
  if (seedsim >= 0):
    # print("seedsim",seedsim)
    # random.seed(seedsim)
    # random.seed(seedsim)
    np.random.seed(seedsim)
    model.env.seed(seedsim)
  ...
  for episode in range(num_episode):
    ...
    obs = model.env.reset_obs()
      
    reward = 0
    action_probs = np.array([0,0,0,0])
    action = 0
    ...
    # while 1:
      ...
      if obs.shape == model.vae.input_dim: ### running in real environment
        obs = config_custom2dmaze.adjust_obs(obs)
        reward = config_custom2dmaze.adjust_reward(reward)
      ...
      if generate_data_mode:
        action = config_custom2dmaze.generate_data_action(t=t, env = model.env)
      else:
        action_probs_soft = model.get_action(controller_obs, t=t, add_noise=ADD_NOISE)
      
      action = np.argmax(action_probs_soft)
      #action = np.random.choice(len(action_probs_soft), p=action_probs_soft)
      epsilon = 0.1
      # initial_epsilon = 0.9
      # final_epsilon = 0.1
      # exploration_steps = 100
      # epsilon_step = (initial_epsilon - final_epsilon) / exploration_steps
      # epsilon = initial_epsilon - t * epsilon_step
      
      # if render_mode:
      #   action = np.argmax(action_probs_soft)
      # else :
      #   if np.random.rand() < epsilon:
      #       action = np.random.choice(len(action_probs_soft))
      #   else:
      #       action = np.argmax(action_probs_soft)
  ...
  return reward_list, t_list, timestep1
...
def main(args):
  ...
  if dream_mode:
    ...
    model.make_env(env_name,seedmake_env=-1, render_mode=render_mode)
  ...
  if final_mode:
    ...
    while(1):
      reward, steps_taken ,timestep1= simulate(model, seedsim=-1, render_mode=render_mode, num_episode=1000, max_len = max_length, generate_data_mode = generate_data_mode)
      # print(f'timestep1:{timestep1}')
      print ("terminal reward", reward, "average steps taken", np.mean(steps_taken)+1)
      print ("terminal reward", reward, "average steps taken", np.mean(steps_taken)+1)
      break
      # print("add_argseed,final_mode", the_seed)
if __name__ == "__main__":
  parser = argparse.ArgumentParser(description=('View a trained agent'))
  parser.add_argument('env_name', type=str, help='car_racing etc - this is only used for labelling files etc, the actual environments are defined in train_envs in config_custom2dmaze.py')
  ...
  parser.add_argument('--generate_data_mode', action='store_true', help='uses the pick_random_action function from config_custom2dmaze')
