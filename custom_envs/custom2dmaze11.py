...
if __name__ == '__main__':
    ...
    for _ in range(300):
        ...
        env.render_agent()
        plt.draw()
        plt.pause(0.1)  # 状態が表示される時間（必要に応じて調整）
    plt.pause(5)
    # import matplotlib.pyplot as plt
    ...
    # env = EasyMaze()
    # obs = env.reset()
    # plt.imshow(env.render(), cmap='viridis')  # エージェントの初期位置を表示
    # plt.show()

    # # エージェントを動かして状態遷移を可視化
    # for _ in range(200):
    #     action = env.action_space.sample()  # ランダムな行動を選択
    #     obs, re, done, info = env.step(action)  # 状態遷移を実行
    #     plt.imshow(env.render(), cmap='viridis')  # 新しい状態を表示
    #     plt.pause(0.1)  # 0.1秒間の一時停止
    #     if done:
    #         obs = env.reset()  # ゴールに達したらリセット
