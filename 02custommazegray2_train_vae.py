...
from vae.arch_custommazegray2z32k3conv1 import VAE
import argparse
import numpy as np
import config
import os
...
def import_data(load, M,i):
  ...
  # print(filelist)
  data = np.zeros((M*load, SCREEN_SIZE_X, SCREEN_SIZE_Y, 1), dtype=np.float32)
  idx = 0
  file_count = 0
  # M_obstotal=0
...
def main(args):
  ...
  vae = VAE()
  checkpoint_path=f'./vae/weights_custommaze_gray2{N}.{epochs}z32k3conv1r100000lr0001.ckpt'
    ...
    for i in range(loadnum):
      if not new_model:
        try:
          ...
          vae.set_weights(f'./vae/vae_weights/weights_custommaze_light{N}.10z16flat.ckpt')
  ...
  vae.save_weights(f'./vae/vae_weights/weights_custommaze_gray2{N}.{epochs}z32k3conv1r100000lr0001.ckpt')
