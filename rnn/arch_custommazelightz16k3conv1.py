...
class RNN():
	...
	def _build(self):
		...
		mdn_model = mdn(lstm_output_model)#(None,Œn—ñ’·,256)‚ğ(None,Œn—ñ’·,481)‚Ö
		...
		forward = Model([rnn_x] + [state_input_h, state_input_c], [mdn_forward, state_h, state_c])
  		#“ü—Í[(None,None,36), (None,256),(None,256)]AÅŒã‚Ìo—Í(None,None,481),  lstm_output_forward‚©‚ç‚Ìo—Í(None,256),(None,256)
		...
		def rnn_z_loss(y_true, y_pred):
			...
			#z_true‚ª“¾‚ç‚ê‚½ã‚Å‚Ì•‰‚Ì‘Î”–Ş“x‚ğÅ¬‚É‚·‚é‚æ‚¤‚Èmu,sigma,pai‚ğ„’è
	...
	def get_mixture_coef(self, z_pred):
		...
		log_pi, mu, log_sigma = tf.split(z_pred, 3, 1)
  		#z_pred (ƒGƒsƒ\[ƒh”~‚Q‚X‚X~‚R‚QA15)¨(ƒGƒsƒ\[ƒh”~‚Q‚X‚X~‚R‚QA‚T)~‚R
		#axis=1‚æ‚è2ŸŒ³–Ú‚Ì²i—ñ•ûŒüj‚ğŠî€‚É•ªŠ„(•ªŠ„”‚R)
		log_pi = log_pi - K.log(K.sum(K.exp(log_pi), axis = 1, keepdims = True)) # axis 1 is the mixture axis¬‡ŒW”‚Ì‘Î”
