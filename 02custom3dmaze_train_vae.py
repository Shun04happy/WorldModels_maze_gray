...
from vae.arch_custom3dmaze import VAE
import argparse
import numpy as np
import config
import os
...
DIR_NAME = './data/custom3dmaze_test/'
...
def import_data(load, M,i):
  ...
  # print(filelist)
  data = np.zeros((M*load, SCREEN_SIZE_X, SCREEN_SIZE_Y, 3), dtype=np.float32)
  idx = 0
  file_count = 0
  # M_obstotal=0
...
def main(args):
  ...
  vae = VAE()
  checkpoint_path=f'./vae/vae_weights/weights_custom3dmaze{N}.{epochs}.ckpt'
    ...
    for i in range(loadnum):
      if not new_model:
        try:
          ...
          vae.set_weights(f'./vae/vae_weights/weights_custom3dmaze_test{N}.{epochs}z32.ckpt')
      ...
      vae.save_weights(f'./vae/vae_weights/weights_custom3dmaze_test{N}.{epochs}z32.ckpt')
