...
def main(args):
    ...
    if run_all_envs:
        envs_to_generate = config_custom2dmaze.train_envs
    else:
        ...
        while s < total_episodes:
            ...
            while t < time_steps:  # and not done:
                if t % action_refresh_rate == 0:
                    action = config_custom2dmaze.generate_data_action(t, env)  # <2>
                ...
                observation = config_custom2dmaze.adjust_obs(observation)
                obs_sequence.append(observation.copy())
                action_sequence.append(action)
                reward_sequence.append(reward)
                done_sequence.append(done)
            ...
            s = s + 1
            if (obs_sequence[s-1]==obs_sequence[s-2]).all():
                    print(f'itti{itti}')
                    itti+=1
            # if (action_sequence[s-1]==action_sequence[s-2]).all():
            #         print(f'aitti{aitti}')
            #         aitti+=1
        env.close()
