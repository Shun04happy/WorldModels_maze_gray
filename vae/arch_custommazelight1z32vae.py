...
class VAE():
    ...
    def _build(self):
        ...
        vae_z_mean = Dense(LATENT_DIM,activation='relu', name='mu')(flattened_input)
        vae_z_log_var = Dense(LATENT_DIM, activation='relu' ,name='log_var')(flattened_input)
        ...
        encoder = Model(inputs=input_layer, outputs= [vae_z_mean, vae_z_log_var, vae_z], name='encoder')
        decoder = Model(inputs=decoder_input, outputs=output_layer, name='decoder')

        vae_full = VAEModel(encoder, decoder,R_LOSS_FACTOR )
        ...
        return (vae_full,encoder,decoder)
