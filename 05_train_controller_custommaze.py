...
from model_custommaze import make_model, simulate
...
def initialize_settings(sigma_init=0.1, sigma_decay=0.9999, init_opt = ''):
  ...
  population = num_worker * num_worker_trial
  filebase = './log/'+env_name+'.'+optimizer+'.'+str(num_episode)+'.'+str(population)+'custommazetest'
  controller_filebase = './controller/'+env_name+'.'+optimizer+'.'+str(num_episode)+'.'+str(population)+'custommazetest'
  ...
  if len(init_opt) > 0:
    ...
    if optimizer == 'ses':
      ...
      cma = CMAES(num_params,# number of model parameters 288*3+3=867
        sigma_init=sigma_init,
        popsize=population)
      es = cma
    elif optimizer == 'pepg':
...
def worker(weights, seed, max_len, new_model):
  ...
  reward_list, t_list , timestep1= simulate(
    new_model
  ...
  t = np.mean(t_list)
  print(f'finish:{rank} timestep:{timestep1} reward:{reward:.2f}')
  return reward, t
...
def evaluate_batch(model_params, max_len):
  ...
  for current_env_name in config_custom2dmaze.train_envs:
...
def leader():
  ...
  #if len(config_custom2dmaze.train_envs) == 1:
  current_env_name = config_custom2dmaze.train_envs[0]
  ...
  while True:
    ...
    for current_env_name in config_custom2dmaze.train_envs:
      ...
      if len(config_custom2dmaze.train_envs) > 1:
        print('completed environment {} of {}'.format(e_num, len(config_custom2dmaze.train_envs)))
      e_num += 1
      
    reward_list = reward_list / len(config_custom2dmaze.train_envs)
    time_list = time_list / len(config_custom2dmaze.train_envs)
