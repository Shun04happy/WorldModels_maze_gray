...
def random_batch(filelist, batch_size):
	...
	for i in indices:
		try:
			...
			# z = z + np.exp(log_var/2.0) * np.random.randn(*s)
...
def main(args):
	...
	if not new_model:
		try:
			rnn.set_weights(f'./rnn/weights_custom2dmaze_lightz16flat.{steps}.{batch_size}.ckpt')
		except:
	...
	for epoch in range(epochs):
		...
		for step in range(steps):
			...
			if step == 0:
				np.savez_compressed(ROOT_DIR_NAME + f'rnn_files_custom2dmaze_grayz32k3conv4g5h256.{steps}.{batch_size}.{epochs}ckpt.npz', rnn_input = rnn_input, rnn_output = rnn_output)
			...
			if step % 10 == 0:
				...
				rnn.model.save_weights(f'./rnn/rnn_weights/weights_custom2dmaze_grayz32k3conv4g5h256.{steps}.{batch_size}.{epochs}.ckpt')
		# rnn.model.save_weights(f'./rnn/weights{steps}.{batch_size}.h5')
