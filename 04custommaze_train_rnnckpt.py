...
from rnn.arch_custommazelightz32k3conv1 import RNN
import argparse
import numpy as np
import os

ROOT_DIR_NAME = './data/'
SERIES_DIR_NAME = './data/custom2dmaze_series_light2/'
...
def main(args):
	...
	if not new_model:
		try:
			rnn.set_weights(f'./rnn/weights_custom2dmaze_lightz32k3conv1g5h256.{steps}.{batch_size}.ckpt')
		except:
			print(f"Either set --new_model or ensure ./rnn/weights_custom2dmaze{steps}.{batch_size}.ckpt exists")
			raise
	...
	for epoch in range(epochs):
		...
		for step in range(steps):
			...
			if step == 0:
				np.savez_compressed(ROOT_DIR_NAME + f'rnn_files_custom2dmaze_lightz32k3conv1g5h256.{steps}.{batch_size}.{epochs}ckpt.npz', rnn_input = rnn_input, rnn_output = rnn_output)
			...
			if step % 10 == 0:
				...
				rnn.model.save_weights(f'./rnn/rnn_weights/weights_custom2dmaze_lightz32k3conv1g5h256.{steps}.{batch_size}.{epochs}.ckpt')
		# rnn.model.save_weights(f'./rnn/weights{steps}.{batch_size}.h5')
...
if __name__ == "__main__":
		...
		parser.add_argument('--N',default = 5000, help='number of episodes to use to train')
